...
require_line "$systemd_dir/backlit-compositor.service" "ExecStart=/usr/bin/backlit-compositor --backend=drm --runtime=smithay --socket=backlit-0 --serve"
...
"$bin_dir/backlit-session" \
  ...
  --desktop-dir "$install_root/usr/share/applications" \
