...
session_bin="${BACKLIT_SESSION_BIN:-}"
...
if [ -n "$session_bin" ]; then
  test -x "$session_bin" || fail "session binary is not executable: $session_bin"
  "$session_bin" \
    --backend=headless \
...
cat > "$out_dir/manifest.json" <<EOF
{
  ...
  "artifacts": {
    ...
    "session_binary": "${session_bin:-cargo run -p backlit-session}",
