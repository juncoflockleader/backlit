...
- Dedicated DRM session verifier that passes as expected-blocked in nested desktops such as the Parallels Ubuntu GUI session, but requires `backlit-session --require-drm-master-present` first commit/vblank plus GUI/service/launch/clean-exit evidence from a real TTY or display-manager owned Backlit session.
...
- Smithay compositor runtime smoke that builds `backlit-compositor` with the optional Smithay backend feature and, on launch-ready Linux hosts, runs `backlit-compositor --backend=drm --runtime=smithay --scripted-client`, `backlit-compositor --backend=drm --runtime=smithay --smithay-client-smoke`, `backlit-compositor --backend=drm --runtime=smithay --drm-first-present-probe`, plus a bounded `--runtime=smithay --serve` socket lifecycle through the same `CompositorRuntime` trait, proving scripted app lifecycle, a generated `wayland-client` registry/bind/surface/xdg-toplevel configure/ack/two-commit lifecycle, wl_seat keyboard/pointer capability advertisement, persistent Smithay seat keyboard/pointer handles, keyboard/pointer dispatch through those handles, compositor-owned DRM/KMS first-present framebuffer and plane-state readiness with either commit/vblank or the expected DRM-master boundary, service-ready bootstrap, demo-client socket traffic, core Wayland protocol globals, Wayland display dispatch, and calloop event-loop ticks can route through a Smithay Wayland display/listening-socket/client insertion boundary.
