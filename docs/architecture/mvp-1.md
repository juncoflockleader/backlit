...
- `scripts/verify-smithay-runtime-probe.sh` compiles `backlit-compositor-backend` with `--features smithay-backend` on Linux and requires the Smithay DRM/KMS, libinput, libseat/logind, Wayland frontend, and calloop components to be linkable. On the Parallels launch-ready guest it also requires `smithay_runtime_probe: true`; on non-Linux or non-launch-ready hosts it writes expected-blocked evidence.
...
- `scripts/verify-drm-session-smoke.sh` runs the full DRM session path with GUI verification, launch spawn, compositor/shell/settings service probes, demo-client launch through the session-started compositor socket with app id preserved, and clean shutdown on launch-ready Linux hosts.
- `scripts/verify-mvp1-contract.sh` collects the MVP 1 readiness evidence into one gate. On non-launch-ready hosts it requires expected-blocked DRM artifacts; inside the Parallels Ubuntu GUI guest it requires launch-ready DRM preflight, Smithay runtime probe, DRM session smoke, package-installed replay, system-installed replay, nested Wayland, launch-performance, resource-budget, compositor-runtime, and compositor-socket evidence.
