...
- `backlit-surface --verify` verifies the policy-level xdg-shell toplevel contract: create, configure, ack, map into window policy, focus, maximize, fullscreen, close request, and clean removal. `backlit-compositor -- --smoke-test` also maps a configured xdg toplevel into the headless compositor backend frame so the compositor entrypoint consumes the same window lifecycle.
