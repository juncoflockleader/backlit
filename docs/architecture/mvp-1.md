...
- `scripts/verify-compositor-socket.sh` runs `backlit-compositor --socket=... --serve --serve-for-ms=500` with a private `XDG_RUNTIME_DIR`, proving the compositor publishes the configured Unix socket, accepts `backlit-demo-client`, maps its announced surface and app id into window policy, processes a client damage event, closes the surface, removes the managed window, disconnects the client, and removes the socket on bounded service exit.
