...
- `backlit-session --verify-desktop-launch` verifies the same discovered-entry launch path from the session process; staged, Debian root, and real system-install checks use the packaged Settings desktop entry and resolve `backlit-settings` next to the installed session binary.
