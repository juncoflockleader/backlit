...
It measures the deterministic headless GUI render path, the in-memory headless backend present path, no-redraw idle frames, targeted surface damage, and a 60-frame drag pacing loop. This does not prove real compositor latency, but it catches early regressions while nested Wayland and DRM/KMS backends are still being built.
