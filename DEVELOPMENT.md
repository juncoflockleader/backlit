...
./scripts/verify-parallels-post-repair-readiness.sh
./scripts/verify-parallels-ubuntu-health.sh
./scripts/verify-parallels-linux-e2e.sh
./scripts/verify-parallels-dedicated-drm-e2e.sh
./scripts/verify-mvp-complete.sh
./scripts/render-parallels-gui-preview.sh
```

The `verify-parallels-mvp-e2e.sh` orchestrator is the one-command MVP path: it writes `target/parallels-mvp-e2e/manifest.json`, requires a clean source tree whose `HEAD` matches its upstream, verifies the local headless GUI preview, runs both health probes, stops before guest mutation when health fails, and otherwise runs normal Parallels E2E, dedicated DRM E2E, and the final MVP-complete audit in order. After repairing a read-only VM, use `verify-parallels-post-repair-readiness.sh` for a fast non-mutating source and health check before starting the full orchestrator. The health verifier is a non-destructive preflight for Parallels itself: it checks that `prlctl` can see the VM, that the configured guest user is executable, and that the Ubuntu root filesystem and `/tmp` are writable before the longer E2E scripts try to upload, install, or build anything. If it reports `guest-root-read-only`, follow `docs/runbooks/parallels-ubuntu-readonly.md` before trying E2E again. Both Parallels E2E wrappers run this health verifier first and export its manifest under their output directories before doing any guest mutation. The E2E runner verifies the full Ubuntu path, runs an opt-in root system-install smoke inside the disposable guest, then copies a compact evidence bundle back to `target/linux-e2e-parallels/`: the Parallels health manifest, guest E2E manifest, GUI smoke/preview manifests, launch-readiness, DRM-master boundary, DRM session-smoke, launch-performance, and resource-budget manifests, Debian package build/install/system-install manifests, installed-package replay manifests, nested Wayland manifest, MVP 0/MVP 1 contract manifests, and the E2E GUI preview image. The package-install and system-install checks also run the scripted session replay from the installed `backlit-session` binary, so launcher and app-switcher overlay frames are proven after packaging; they also prove compositor socket clients and the installed Settings desktop entry map into managed policy with app id and focus preserved. The wrapper retries guest `git fetch` and `git clone` commands to tolerate transient Ubuntu guest DNS failures before treating checkout as a real E2E failure, and preflights that the Ubuntu guest is writable before uploading scripts. The dedicated DRM runner is opt-in because it builds the Debian packages in the guest, installs the `fastgui-core` runtime closure, briefly switches the guest to a real TTY with `systemd-run`, `PAMName=login`, and `TTYPath=/dev/tty3`, runs the dedicated verifier with `BACKLIT_DEDICATED_DRM_SESSION_BIN=/usr/bin/backlit-session`, then restores the previous console and purges the packages after requiring first commit/vblank, GUI verification, compositor services, desktop launch, demo-client, and clean exit. It exports the package-build manifest, dpkg logs, dedicated manifest, health manifest, and screenshot; override the target TTY with `BACKLIT_PARALLELS_DEDICATED_TTY=/dev/ttyN` if needed. After both Parallels runners pass on the same pushed commit, run `./scripts/verify-mvp-complete.sh target/mvp-complete target/linux-e2e-parallels target/parallels-dedicated-drm-e2e` from a clean worktree to audit exported health first, followed by launch-performance, resource-budget, PNG GUI preview, package-install, and dedicated-DRM evidence before calling MVP 1 complete. The preview runner renders the Backlit preview inside Ubuntu, copies the generated PPM/session logs/manifest back to `target/gui-preview-parallels/`, and converts the PPM to a local PNG on macOS when possible.
...
```bash
BACKLIT_PARALLELS_VM="Ubuntu 22.04.2 ARM64" ./scripts/verify-parallels-ubuntu-health.sh
...
./scripts/verify-dedicated-drm-session.sh
...
The compositor-runtime verifier runs `backlit-compositor --scripted-client --scripted-client-preview target/compositor-runtime/scripted-client-policy-preview.ppm --serve --serve-for-ms=25`, maps two app-like surfaces through the `CompositorRuntime` backend trait, verifies they become managed policy windows, writes and verifies a GUI preview frame from that compositor path, verifies targeted damage and idle no-redraw behavior, closes one surface, disconnects the client, and checks the cleanup frames. The manifest records the baseline runtime backend as `headless-compositor`. The Smithay compositor runtime verifier runs `backlit-compositor --backend=drm --runtime=smithay --scripted-client --scripted-client-preview target/smithay-compositor-runtime/smithay-normal-runtime-real-client-frame.ppm`, `backlit-compositor --backend=drm --runtime=smithay --smithay-client-smoke`, and a bounded `--runtime=smithay --serve` service socket with `--features smithay-backend`; on launch-ready Linux it requires the scripted lifecycle, a generated `wayland-client` registry/bind/surface/xdg-toplevel configure/ack/two-commit lifecycle with server-side xdg title and app-id metadata callbacks, a real 320x240 `wl_shm` buffer attach observed through Smithay buffer dimensions, a normal Smithay runtime frame that presents and CPU-composes the live snapshot pixels through Backlit `WindowPolicy` geometry with sample verification, a generated real xdg surface lifecycle that resizes to 420x300, commits the resized SHM buffer, unmaps, receives xdg close, destroys protocol objects, disconnects, and leaves no stale Backlit policy windows, generated real-client input where two focused `wl_shm` xdg clients receive `wl_pointer` enter/motion/button and `wl_keyboard` enter/key events with focus switching to the second client plus a compositor shortcut-filter probe that is intercepted before forwarding, wl_seat keyboard/pointer capabilities observed by the generated client, persistent Smithay seat keyboard/pointer handles, keyboard/pointer dispatch counters from those handles, a Backlit policy window mapped from that observed title/app id and buffer geometry, service-ready bootstrap, and demo-client socket lifecycle to report `runtime_backend: "smithay-compositor-runtime"` plus `implementation: "smithay-compositor-runtime"` in the launch plan while proving that the Smithay runtime registers 10 globals and that a real client sees the 7 MVP globals: `wl_compositor`, `wl_shm`, `xdg_wm_base`, `zxdg_output_manager_v1`, `wp_viewporter`, `wp_presentation`, and `zwp_linux_dmabuf_v1` v4 or newer. It also continues to require `wl_subcompositor`, `wl_seat`, keyboard/pointer capabilities, inserted Wayland clients, Wayland display dispatch, calloop ticks, `smithay_normal_runtime_live_snapshot_frame: true`, `smithay_normal_runtime_real_pixels: true`, `smithay_real_surface_lifecycle: true`, `smithay_real_xdg_resize_commit: true`, `smithay_real_xdg_unmap_cleanup: true`, `smithay_real_xdg_close_disconnect: true`, `smithay_policy_lifecycle_cleanup: true`, `smithay_real_client_input: true`, `smithay_real_pointer_input: true`, `smithay_real_keyboard_input: true`, `smithay_real_input_focus_routing: true`, and `smithay_shortcut_filter_preserved: true` on the same scripted and service-socket presents. The compositor-socket verifier and session service probe additionally preserve demo-client app ids from socket announcements into managed window policy. The socket verifier also sends a multi-client management and lifecycle sequence through the service boundary: announce a first surface, announce and focus a second surface, move it, resize it, maximize it into the work area, fullscreen it across the output, damage the focused surface, close it, remove its policy window, fall focus back to the first window, and disconnect the closed client. It is a bounded proof for the app-window lifecycle and geometry contract that the future full Smithay protocol/runtime loop must preserve.
...
./scripts/verify-session-replay.sh
./scripts/verify-session-clean-exit.sh
./scripts/verify-drm-master-boundary.sh
./scripts/verify-drm-session-smoke.sh
./scripts/verify-dedicated-drm-session.sh
./scripts/verify-systemd-activation.sh
...
These write `target/smithay-runtime-probe/manifest.json`, `target/smithay-compositor-runtime/manifest.json`, `target/launch-readiness/manifest.json`, `target/session-launch/manifest.json`, `target/session-replay/manifest.json`, `target/session-clean-exit/manifest.json`, `target/drm-master-boundary/manifest.json`, `target/drm-session-smoke/manifest.json`, `target/dedicated-drm-session/manifest.json`, `target/systemd-activation/manifest.json`, and `target/mvp1-contract/manifest.json`. On macOS or headless CI they can pass with DRM expected-blocked; inside the Parallels Ubuntu GUI VM the wrapper maps the active `parallels` logind session before running E2E, so the manifests should report a user-owned runtime, active local session, accessible DRM card, `input_broker_ready: true`, DRM expected-ready, and ready. Direct `/dev/input/event*` access is recorded separately; when it is unavailable, the manifest should report `input_broker_mode: "logind-libseat"`.

The Smithay runtime probe compiles `backlit-compositor-backend` with `--features smithay-backend`, verifies Smithay DRM/GBM/EGL/GLES/libinput/libseat/calloop components are linkable, resolves the selected DRM card into a Smithay `DrmNode`, opens that primary DRM card through `DrmDevice`, enumerates KMS CRTCs, connectors, connected modes, and planes, selects a connected connector plus mode/CRTC/primary-plane scanout target tuple, inserts the DRM notifier into calloop, dispatches that event loop once, creates a Smithay `DrmSurface` for the selected target, verifies the surface's pending connector, mode, CRTC, and primary-plane metadata, allocates a scanout-sized dumb framebuffer, maps and fills it, attaches it as a DRM framebuffer, prepares the selected primary plane for first present, attempts to validate that state with Smithay `DrmSurface::test_state`, records either a successful first modeset commit with vblank event or the expected DRM-master permission denial from a nested desktop session, releases the framebuffer before surface teardown, pauses the DRM device before dropping the surface, opens the matching render node, creates a Smithay GBM device, GBM allocator, EGL display, EGL context, and GLES renderer from that node, renders a 16x16 offscreen GLES frame, copies it back to CPU memory, verifies a red RGBA sample, creates a Smithay libseat session for the active seat, creates a libinput udev context through that session, assigns it to the seat, inserts both the libseat notifier and libinput backend into calloop, dispatches that event loop once, classifies libinput events into total/device/keyboard/pointer/touch/gesture/tablet/switch/special counters, creates a Smithay Wayland display, binds a listening socket, connects/accepts/inserts a local client, dispatches/flushes clients, runs one calloop dispatch, and requires `smithay_runtime_probe: true` plus `smithay_runtime_bootstrap: true` on launch-ready Linux.

The DRM-master boundary verifier wraps that probe into the session-ownership contract: it confirms the packaged session entry and compositor service use the DRM backend, records either true first-present commit/vblank evidence or the expected DRM-master denial, and marks `dedicated_session_required: true` when the current desktop session cannot own modesetting; set `BACKLIT_REQUIRE_DRM_MASTER_PRESENT=1` when running from a real dedicated Backlit session and you want denial to fail. The dedicated DRM session verifier uses that boundary as a guard: it writes expected-blocked evidence in Parallels/nested desktops and generates `dedicated-drm-handoff.sh` beside its manifest. The handoff artifact prints the current session id, seat, type, runtime directory, and Wayland display before requiring a seat-owner TTY or display-manager Backlit session with DRM master. Run that handoff script, or run `BACKLIT_REQUIRE_DEDICATED_DRM_SESSION=1 BACKLIT_REQUIRE_DRM_MASTER_PRESENT=1 ./scripts/verify-dedicated-drm-session.sh target/dedicated-drm-session-acceptance`, from a dedicated TTY or display-manager owned Backlit session to require first commit/vblank plus GUI, service, launch, and clean-exit evidence. Set `BACKLIT_DEDICATED_DRM_SESSION_BIN=/usr/bin/backlit-session` to require the installed package binary instead of the default source-built `target/debug/backlit-session`.

On the Parallels Ubuntu guest, `./scripts/verify-parallels-dedicated-drm-e2e.sh` automates that handoff by fetching the configured branch, building the Debian packages, installing the `fastgui-core` runtime closure, running `verify-dedicated-drm-session.sh` inside an active `parallels` logind session on `/dev/tty3` with `/usr/bin/backlit-session`, restoring the prior console, purging the packages, and exporting `target/parallels-dedicated-drm-e2e/manifest.json` plus package logs and the dedicated-session screenshot. This is the true package-installed DRM-master acceptance path for the VM; the normal GUI-session E2E still records the nested desktop DRM-master boundary separately.

The Smithay compositor runtime verifier compiles `backlit-compositor` with the same feature and requires `backlit-compositor --backend=drm --runtime=smithay --scripted-client --scripted-client-preview <path>`, `backlit-compositor --backend=drm --runtime=smithay --smithay-client-smoke`, `backlit-compositor --backend=drm --runtime=smithay --drm-first-present-probe`, plus a bounded `--runtime=smithay --serve` demo-client socket lifecycle to pass on launch-ready Linux, including `smithay_runtime_launch_plan: true`, `smithay_core_protocol_globals: true`, `smithay_mvp_protocol_globals: true`, `smithay_seat_global: true`, `smithay_keyboard_pointer_capabilities: true`, `smithay_input_seat_handles: true`, `smithay_input_seat_dispatch: true`, `smithay_real_wayland_client: true`, `smithay_real_wayland_metadata: true`, `smithay_real_shm_buffer: true`, `smithay_normal_runtime_live_snapshot_frame: true`, `smithay_normal_runtime_real_pixels: true`, `smithay_real_surface_lifecycle: true`, `smithay_real_xdg_resize_commit: true`, `smithay_real_xdg_unmap_cleanup: true`, `smithay_real_xdg_close_disconnect: true`, `smithay_policy_lifecycle_cleanup: true`, `smithay_real_client_input: true`, `smithay_real_pointer_input: true`, `smithay_real_keyboard_input: true`, `smithay_real_input_focus_routing: true`, `smithay_shortcut_filter_preserved: true`, `smithay_real_wayland_policy_window: true`, `smithay_event_loop_runtime: true`, `smithay_drm_first_present_probe: true`, and `smithay_event_loop_service_socket: true` evidence from the runtime's Wayland display, runtime-aware launch plan, core protocol globals, the MVP protocol globals and minimum versions, wl_seat keyboard/pointer capabilities, persistent Smithay seat keyboard/pointer handles, keyboard/pointer dispatch through those handles, xdg-toplevel client lifecycle, xdg title/app-id callbacks, real wl_shm buffer commit dimensions, normal frame presentation of live snapshot pixels, real generated xdg resize/unmap/close/disconnect lifecycle cleanup, real generated pointer/keyboard delivery with focus routing, compositor shortcut filtering, compositor-owned KMS first-present boundary probe, Backlit policy-window mapping, and calloop dispatch counters. `backlit-compositor-backend`, `backlit-compositor`, and `backlit-session` also emit backend launch-plan events after preflight; explicit DRM session and compositor launch plans now report `implementation: "smithay-compositor-runtime"` so the packaged session path and the runtime verifier use the same implementation evidence.

The session launch verifier also checks that `packaging/sessions/backlit.desktop` resolves to `backlit-session --backend=drm --activate-systemd`, that `backlit-session --preflight-only` exits cleanly for launchable backends, and that `backlit-session --verify-systemd-units` accepts the `backlit-session.target` user target plus the dry-run `systemctl --user import-environment/start/stop` launch plan for compositor, shell, notification, and settings services. The packaged service units run those services with unbounded `--serve`; bounded `--serve-for-ms` is reserved for automation. The import plan must carry `XDG_RUNTIME_DIR`, `XDG_SESSION_ID`, `XDG_SEAT`, `XDG_SESSION_TYPE`, `WAYLAND_DISPLAY`, `XDG_CURRENT_DESKTOP`, and `DESKTOP_SESSION` into the user manager so DRM/logind preflight sees the same launch context as the session entrypoint. The systemd activation verifier executes that import/start/stop sequence through a fake `systemctl` without mutating the host user manager. The session replay verifier captures deterministic frames for visible focus/launch/move/resize/snap/workspace state changes. The session clean-exit verifier checks requested shutdown from the headless session path. The DRM session smoke verifier builds `backlit-session` with the Smithay feature and runs the full `backlit-session --backend=drm --verify-drm-first-present` path with GUI verification, terminal spawn verification, compositor/shell/settings service probes, demo-client launch through the session-started compositor socket, launch-plan verification, and clean shutdown when the host is launch-ready. For reference hardware or a dedicated TTY/display-manager session, add `--require-drm-master-present` to make first commit/vblank mandatory instead of accepting the nested-session DRM-master boundary. The MVP 1 contract verifier gathers this evidence into one readiness gate and stays explicit that the real compositor loop is still being brought up.
...
The DRM session smoke also launches its sibling compositor service with `--runtime=smithay` for `--backend=drm` and requires the session log plus manifest to report `session_compositor_smithay_runtime: true`, `session_compositor_smithay_protocol_globals: true`, `session_compositor_smithay_input_seat_handles: true`, and `session_compositor_smithay_input_seat_dispatch: true`.

The Smithay compositor runtime also installs libseat and libinput event sources into its calloop loop on launch-ready Linux. The Smithay compositor manifest requires `smithay_runtime_launch_plan: true` so explicit `backlit-compositor --backend=drm --runtime=smithay` launches report `implementation: "smithay-compositor-runtime"` instead of the generic backend harness label. The Smithay and DRM session manifests require `smithay_input_sources: true`, `smithay_input_event_loop: true`, `smithay_input_seat_handles: true`, `smithay_input_seat_dispatch: true`, `session_compositor_smithay_input_sources: true`, `session_compositor_smithay_input_event_loop: true`, `session_compositor_smithay_input_seat_handles: true`, and `session_compositor_smithay_input_seat_dispatch: true` before treating the runtime/session path as input-ready.
