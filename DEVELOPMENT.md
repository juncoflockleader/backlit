...
The DRM-master boundary verifier wraps that probe into the session-ownership contract: it confirms the packaged session entry and compositor service use the DRM backend, records either true first-present commit/vblank evidence or the expected DRM-master denial, and marks `dedicated_session_required: true` when the current desktop session cannot own modesetting; set `BACKLIT_REQUIRE_DRM_MASTER_PRESENT=1` when running from a real dedicated Backlit session and you want denial to fail. The dedicated DRM session verifier uses that boundary as a guard: it writes expected-blocked evidence in Parallels/nested desktops, but from a dedicated TTY or display-manager owned Backlit session it runs `backlit-session --require-drm-master-present` and requires first commit/vblank plus GUI, service, launch, and clean-exit evidence.
