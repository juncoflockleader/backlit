...
The DRM-master boundary verifier wraps that probe into the session-ownership contract: it confirms the packaged session entry and compositor service use the DRM backend, records either true first-present commit/vblank evidence or the expected DRM-master denial, and marks `dedicated_session_required: true` when the current desktop session cannot own modesetting; set `BACKLIT_REQUIRE_DRM_MASTER_PRESENT=1` when running from a real dedicated Backlit session and you want denial to fail. The dedicated DRM session verifier uses that boundary as a guard: it writes expected-blocked evidence in Parallels/nested desktops and generates `dedicated-drm-handoff.sh` beside its manifest. The handoff artifact prints the current session id, seat, type, runtime directory, and Wayland display before requiring a seat-owner TTY or display-manager Backlit session with DRM master. Run that handoff script, or run `BACKLIT_REQUIRE_DEDICATED_DRM_SESSION=1 BACKLIT_REQUIRE_DRM_MASTER_PRESENT=1 ./scripts/verify-dedicated-drm-session.sh target/dedicated-drm-session-acceptance`, from a dedicated TTY or display-manager owned Backlit session to require first commit/vblank plus GUI, service, launch, and clean-exit evidence.

On the Parallels Ubuntu guest, `./scripts/verify-parallels-dedicated-drm-e2e.sh` automates that handoff by fetching the configured branch, running `verify-dedicated-drm-session.sh` inside an active `parallels` logind session on `/dev/tty3`, restoring the prior console, and exporting `target/parallels-dedicated-drm-e2e/manifest.json` plus the dedicated-session screenshot. This is the true DRM-master acceptance path for the VM; the normal GUI-session E2E still records the nested desktop DRM-master boundary separately.
