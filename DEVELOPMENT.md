...
The shell smoke path runs `backlit-shell --component=all --verify` and checks the MVP chrome state: wallpaper, panel clock/battery indicators, NetworkManager-backed network status, PipeWire-backed audio status, workspace indicator, launcher targets, and app switcher entries.
